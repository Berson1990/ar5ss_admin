...
(function () {
    ...
    angular.module('App').controller('FinancialSituation', FinancialSituation);
    FinancialSituation.$inject = [
        '$scope',
        '$state',
        'alertify',
        'SuppliersRepository',
    ...
    function FinancialSituation($scope, $state, alertify, suppliersRepository, ar5sspresntage, $rootScope, dtOptionsBuilder, dtColumnDefBuilder, salesRepository) {
        $scope.Total = 0;
        $scope.AllProduct = false;
        $scope.AllUsers = false;
        $scope.Ar5sspresntage = ar5sspresntage.data[0].percent;
        console.log($scope.Ar5sspresntage);
        ...
        function getBasicData() {
            ...
            suppliersRepository.GetProductSupplierSell(mabdparms).then(function (result) {
                ...
                for (var i = 0; i < $scope.SerachResult.length ; i++){
                    if ($scope.SerachResult[i].Percentage === '0.0') {
                         first = $scope.SerachResult[i].ProductPriceDesc  * $scope.SerachResult[i].ItemsQTY;
                         soecend  = (first * ($scope.Ar5sspresntage/100));
                        console.log(first);
                        console.log(soecend);
                        $scope.Total += first - soecend
                    } else {
                        first = $scope.SerachResult[i].ProductPriceDesc  * $scope.SerachResult[i].ItemsQTY;
                        soecend = (first * ($scope.SerachResult[i].Percentage/ 100));
                        console.log(first);
                        console.log(soecend);
                        $scope.Total +=  first -   soecend;
